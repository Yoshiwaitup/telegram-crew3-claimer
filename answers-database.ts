...
const URL =
  process.env.ANSWERS ||
  "https://docs.google.com/spreadsheets/d/1Dk4qG6d7kjeK_iFbmxJ8Z2gGQRHZ_lsgdqwLiUxXMlg/edit#gid=0";
...
export const writeAnswers = async (data, url = URL) => {
  ...
  if (!sheet || typeof sheet === undefined) {
    ...
    existed = (await sheet.getRows()).map((row) => ({
  ...
  const answers = [...new Set(existed), ...new Set(data.answers)]
    .filter(
      (v, i, a) =>
  ...
  await sheet.addRows(answers).catch((e) => console.log(e.response.data.error));
...
const checkConnection = async (ctx) => {
  try {
    ...
    console.log(
      `Google connection error! Please check provided URL visibility or use default URL.`
    );
  }
};
