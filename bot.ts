...
const COMMUNITY_CATEGORIES_REGEX =
  "([NFT|DAO|Art|Music|Collectibles|Gaming|DeFi|Metaverse|Trading Cards|Infrastructure|Education|Startup|Protocol|Investing|DeSci|new|popular]+)_(.+)";
...
bot
  ...
  .use((ctx, next) => {
    ...
    if (ctx.from.id !== ADMIN)
      return ctx
        .reply("Access Denied!!!")
        .catch((e) => console.log("Access", e));
    if (!ctx.session.accounts) ctx.session.accounts = {};
    return next();
  });
...
*Twitter:* ${
    user.twitterUsername
      ? `[${user.twitterUsername}](https://twitter.com/${user.twitterUsername})`
      : "NONE"
  }
*Discord:* ${
    user.discordHandle
      ? `[${user.discordHandle}](https://discord.com/app)`
      : "NONE"
  }
...
*Profile joined to ${communities.length} communities*: \`${communities
    .map((i) => i.name)
    .join("`, `")}\`
...
const profileButtons = (user) => [
  [
    Key.callback("Daily Connect »", `claim_none_${user.id}`),
    Key.callback("Quiz & questions »", `claim_quiz_${user.id}`),
  ],
  [
    Key.callback("Discord »", `claim_discord_${user.id}`),
    Key.callback("Twitter »", `claim_twitter_${user.id}`),
  ],
  [
...
const getAccountName = (ctx, id) =>
  ctx.session.accounts[id]
    ? ctx.session.accounts[id].crew_user.name ||
      ctx.session.accounts[id].crew_user.discordHandle ||
...
// Main functions
const claimQuestWithReport = async (
  ctx: BotContext,
  id,
  types = ["none"],
  answers = {}
  ...
  if (userCommunities) {
    ...
    await ctx.reply(
      ...
      Keyboard.make([
        [
          Key.callback("« Main menu", "main"),
          Key.callback("To account »", `account_${id}`),
        ],
      ]).inline({ parse_mode: "Markdown", disable_web_page_preview: true })
    );
  }
};
...
const getCookieByPrivateKey = async (ctx, key) => {
  try {
    ...
    await api
      ...
      .then(async (r) => {
        return api
          .post("authentification/wallet/verify-signature", {
            network: 1,
            sessionId: r.data.id,
      ...
      .catch((e) => {
        console.log(e);
        return ctx.reply(
          `Сouldn't get a nonce from Crew3 website - ${signer.address}`
        );
      });
  } catch (e) {
    console.log(e);
...
const join = async (ctx, ids, link) => {
  ...
  for (const id of shuffle(ids)) {
    if (joined < ctx.session.invite.max) {
      const crew = new CrewProfile(ctx.session.accounts[id].crew_headers);
      const state = await crew.joinByReferral(
        ctx.session.invite.subdomain,
        ctx.session.invite.code
      );
      if (state.startsWith("Success")) {
        joined++;
        await ctx
          .reply(
            `*${getAccountName(ctx, id)}:* ${state}\nAutostart claiming...`,
            { parse_mode: "Markdown" }
          )
        ...
        await claimQuestWithReport(
          ctx,
          id,
          ["none", "text", "telegram", "quiz"],
          answers
        );
        await delay(CLAIM_TIMEOUT);
      } else {
        await ctx
          .reply(`*${getAccountName(ctx, id)}:* ${state}`, {
            parse_mode: "Markdown",
          })
          .catch((e) => console.log(e));
  ...
  return ctx
    .reply(`Joined ${joined} accounts complete!`)
    .catch((e) => console.log(e));
};
...
bot
  .hears(/https:\/\/(.*).crew3.xyz\/invite\/([\S]+)\s?(\d*)/, async (ctx) => {
    ...
    const buttons = [
      [
        Key.callback(
          getAccountName(ctx, ctx.session.currentProfile),
          "last_account_" + ctx.session.currentProfile,
          !ctx.session.currentProfile
        ),
        Key.callback(
          `${ctx.match[3] !== "" ? ctx.match[3] : "All"} accounts »`,
          "all_accounts_lastRefLink"
        ),
      ],
      [Key.callback("« Main menu", "main")],
    ];
    const actions = Keyboard.make(buttons).inline({
      parse_mode: "Markdown",
      disable_web_page_preview: true,
    });
    ctx.reply(
      `Select account(s) join to *${ctx.match[1]}* community?`,
      actions
    );
  })
  ...
  .hears(/join (.*)/, async (ctx) => {
    ...
    return ctx
      .reply(`Join community *${community.name}*?`, actions)
      .catch((e) => {});
  })
  .action(
    new RegExp("join_communities_" + COMMUNITY_CATEGORIES_REGEX),
    async (ctx) => {
      const crew = getCrewByMatch(ctx, 2);
      const communities = await crew.getCommunities(
        ctx.match[1],
        COMMUNITY_MAX_PAGE_NUMBER,
        0
      );
      const userCommunitiesSubdomains = (await crew.getUserCommunities()).map(
        (c) => c.subdomain
      );
      const communitiesToJoin = communities.filter(
        (c) => !userCommunitiesSubdomains.includes(c.subdomain)
      );
      await ctx.reply(
        `Found *${communitiesToJoin.length}* out of *${communities.length}* communities that are still not joined.`
      );
  ...
  .action(/join_(.+)/, async (ctx) => {
    for (const id of Object.keys(ctx.session.accounts)) {
      const crew = new CrewProfile(ctx.session.accounts[id].crew_headers);
      await ctx.reply(
        `*${getAccountName(ctx, id)}:* ${await crew.joinCommunity(
          ctx.match[1]
        )}`,
        { parse_mode: "Markdown" }
      );
...
bot
  .hears(/leave (.*)/, async (ctx) => {
    ...
    const actions = Keyboard.make(buttons).inline({ parse_mode: "Markdown" });
    return ctx
      .reply(`Leave community *${community.name}*?`, actions)
      .catch((e) => {});
  })
  .action(/leave_(.+)/, async (ctx) => {
    for (const id of Object.keys(ctx.session.accounts)) {
      const crew = new CrewProfile(ctx.session.accounts[id].crew_headers);
      await ctx.reply(
        `*${getAccountName(ctx, id)}:* ${await crew.leaveCommunity(
          ctx.match[1],
          ctx.session.accounts[id].crew_user
        )}`,
        { parse_mode: "Markdown" }
      );
      await delay(SHORT_TIMEOUT);
...
const main = async (ctx: BotContext, msg = null) => {
  ...
  const actions = Keyboard.make(mainKeyboard(ctx)).inline({
    parse_mode: "Markdown",
    caption: text,
  });

  return ctx.replyWithPhoto(
    "https://aptos-mainnet-api.bluemove.net/uploads/nuclear_8cbc4d5fb5.png",
    actions
  );
};
...
bot
  .command("start", async (ctx) => main(ctx))
  .action("add_profile", async (ctx) => {
    await ctx.replyWithMarkdown(
      `_Method 1:_ Send *private key* of wallet and bot try to log in to Crew3 account!\n\n_Method 2:_ Edit file \`accounts.csv\` in root of bot directory and click this button again.`
    );
    try {
      const accounts = (await fs.readFile("accounts.csv", "utf8")).split("\n");
      if (accounts.length > 0) {
        ctx.reply(
          `File \`accounts.csv\` parsed, found ${accounts.length} keys...`
        );
        let added = 0;
        for (const account of accounts) {
          const address = account.split(/;|,/)[0].toLowerCase();
          if (
            regexEthAddress.test(address) &&
            Object.values(ctx.session.accounts).filter(
              (account) =>
                account["crew_user"].ethWallet.toLowerCase() === address
            ).length === 0
          ) {
        ...
        if (added > 0) return main(ctx, `Done key(s) processing!`);
      }
    } catch (e) {
      console.log(e);
    }
  ...
  .action("all_accounts", async (ctx) => {
    if (ctx.session.accounts) {
      let buttons = [
        [Key.callback("« Main menu", "main")],
        [Key.callback("🌀 Claim Quests »", "claim_none_socials")],
      ];
      buttons = [
        ...buttons,
        ...new Set(
          Object.values(ctx.session.accounts)
            .filter(
              ({ crew_user: account }: any) => account.accounts.length > 1
            )
            .map(({ crew_user: account }: any) => [
              Key.callback(
                account.name ||
                  account.discordHandle ||
  ...
  .hears(regexEthAddress, (ctx) => {
    const accounts = Object.values(ctx.session.accounts).filter(
      (account) =>
        account["crew_user"].ethWallet.toLowerCase() ===
        ctx.match[1].toLowerCase()
    );
    if (accounts.length > 0)
      ctx.reply(
        `Account with wallet \`${ctx.match[1]}\` found!`,
        Keyboard.make([
          [
            Key.callback("« Main menu", `main`),
  ...
  .action(/answers_(.+)/, async (ctx) => {
    ...
    for (const community of await crew.getUserCommunities())
      url = await google.writeAnswers(
        await crew.getCommunityAnswers(community, 0, 500)
      );
    return ctx.reply(`Answers are recorded in a Google SpreadSheets: ${url}`);
  })
  .action("main", async (ctx) => main(ctx))
  .action("invite", async (ctx) =>
    ctx.reply(
      `Send invite link to bot, you can setup how many invitesmust be, example:\n\n*[invite link from crew3]* *[number of invites]*`,
      { disable_web_page_preview: true, parse_mode: "Markdown" }
    )
  )
  .action("leave", (ctx) =>
    ctx.reply("Send to bot command /leave *[subdomain or name]*", {
      parse_mode: "Markdown",
    })
  )
      ...
      await ctx.reply(
        ...
        Keyboard.make([
          [
            ...
            Key.callback(
              "Get invite link »",
              `invite_${community.subdomain}_${user.id}`
            ),
          ],
          [
            Key.callback("« Main menu", `main`),
  ...
  .action(
    new RegExp("communities_" + COMMUNITY_CATEGORIES_REGEX),
    async (ctx) => {
      const crew = getCrewByMatch(ctx, 2);
      const user = await crew.getUser();
      const joined = (await crew.getUserCommunities()).map(
      ...
      await ctx.reply(
        `Want to join ${communities.length} communities from ${ctx.match[1]} category ?`,
        Keyboard.make([
          [
            Key.callback(
              `Join ${communities.length} "${ctx.match[1]}" communities »`,
              `join_communities_${ctx.match[1]}_${ctx.match[2]}`
            ),
          ],
          [
            Key.callback("« Main menu", `main`),
  ...
  .hears(/new (\d*) (\d*)/, async (ctx) => {
    ...
    const communities = await crew.getCommunities(
      "new",
      parseInt(ctx.match[1]) || 3,
      parseInt(ctx.match[2]) || 0
    );
    for (const community of communities) {
      const quests = crew.getRoleQuests(
        await crew.getAllQuests(community.subdomain)
      );
      for (const quest of quests)
        console.log(
          community.subdomain,
          quest.reward,
          quest.validationData,
          quest.condition
        );
  ...
  .command("twitters", async (ctx) => {
    ctx.reply(
      Object.values(ctx.session.accounts)
        ...
        .map(
          ({ crew_user: account }: any) =>
            `https://twitter.com/${account.twitterUsername}`
        )
        .join("\n"),
      { parse_mode: "Markdown" }
    );
  ...
  .action(/claim_community_([none|quiz|any]+)_(.+)_(.+)/, async (ctx) => {
    ...
    for (const id of shuffle(ids)) {
      ...
      const report = await crew.claimQuestsByCommunity(
        community,
        type,
        CLAIM_TIMEOUT,
        answers,
  ...
  .action(/claim_([discord|twitter|social]+)_(.+)/, async (ctx) => {
    ...
    return ctx.reply(
      "Wait for next update for auto-task(join, follow or other social tasks). Now only auto-claim helper supported."
    );
  })
  .action(/delete_(.+)/, async (ctx) => {
    ...
    return main(
      ctx,
      `Account \`${address}\` deleted, to recover send it private key again`
    );
  })
    ...
    if (community) {
      ...
      await ctx.reply(
        ...
        Keyboard.make([
          ...
          [
            Key.callback(
              "Claim from all accounts »",
              `claim_community_any_all_${community.subdomain}`,
              !ctx.session.currentProfile
            ),
          ],
          [Key.callback("« Main menu", `main`)],
...
console.log(
  "Telegram bot successfully launched! Write any message or /start to your bot in telegram..."
);
